...
1. **禁止用业务方案替换抽象论证，或将业务例子混入核心定义。** 核心独立给出抽象类型、构造、组合条件及其事件流；需要举例时，每个业务单独给出具体类型、函数和事件流，检验如何实例化核心。业务反例可以推动公共关系修正，但不能把该业务的字段、政策或状态直接塞入核心，也不能仅将业务名换成泛型字母冒充抽象。模块、流程、数据库表、迁移顺序、代码或测试通过均不能代替这项论证；允许并要求业务特化有自己的精确表达。
...
3. **禁止用静态目录冒充动态语义，或先定组合再让事件流填空。** 列齐对象、消息、状态、函数签名和提交点不等于解释了事件流。新的抽象事件流必须参与推导：计算怎样被解释、结果怎样被消费、后继怎样产生，以及不同顺序、失败和中断怎样改变组合与状态；发现不闭合就修正候选关系，不以“作者提供函数／政策”掩盖缺口。Provider可保持契约明确的黑箱，不要求先枚举内部实现；用户已整体否定的事件流不得择取局部继续拼接。
...
8. **研究前先读失败记录，新失败及时记录。** 开始或恢复研究、派工、修改或整合设计前，实际读取`docs/uta-capability-runtime-design/current-research.md`中的历史会话偏离依据、当前会话失败及所引用记录，不能用compact摘要或记忆替代。按用户已明确给出的失败判定记录原话、实际派工/修改及未解决状态，不重新审判或限缩。道歉、停派工、改文档、补图与补记录均不自动表示已经修复；记录用于改变主线工作，不成为独立交付或无限审计。
