...
- **段池为什么存在——扇出，不是 SIMD** **[设计：E14]**：段池不是为了算得快；fp-11/fp-12 已证明计算侧只需要连续列 + `f64` + validity（洗入的产物），对齐无可测效应（§3.2）。段池的理由是**同一个指标被 N 个策略复用，并派生出一堆计算**：派生 DAG 里每条边都是"一个段被多个消费者读"，共享映射让边的成本为零、每个段只物化一份；任何按消费者复制的方案成本是 O(窗口 × 消费者)，且每个消费进程各持一份副本。独立进程（故障域）与扇出共享同时成立，只有共享段这一条路；进程内 cdylib 没有故障域，按触发复制没有扇出。
...
- **IPC 只交接段**：计算输入 = 段句柄列表 + 位置范围；输出也是一个段 = 一条派生流，同样可被 N 个消费者映射（扇出，E14）。
...
7. 端到端 50 ms 与扇出：真实 iceoryx2 链路上以真实 L2 窗口（fp-12 数据）构造 1 输入 → K 个派生 op → N 个策略消费者的 DAG（K=3, N=10），分项记录 `T_ipc / T_compute / T_writeback` 的 p99 与总内存；对照组为"按消费者 pipe 复制"，给出扇出下两者的成本曲线。`T_ipc` 占比给出 §1.3"IPC 可忽略"成立或否的结论。
