...
- **段池不持久化，持久化行情不是段池的事**：段池只存在于运行期，进程重启后由 `Pooled` 节点从其输入流重新洗入；它没有快照，也不该有。如果需要持久化行情，那是该行情流作为 `Journal` 的事（§2、§6.1），策略（全量 / 抽样 / 仅 gap 标记）归 `Journal` 与保留协议决定 → **[spike S15]**，与段池无关。非行情流不经段池。**[设计：维护者定性]**
...
| `Journal` / 段池 | 记录的载体，持久于 SQLite，由 §6 保留协议管理；行情要持久化就在这里 | 为高性能计算设计的运行期物化，由 `Pooled` 洗入，自有回收，永不持久 | 两套存储，互不派生；只共用 `LogPosition` 标定 | §2 §5.3 §6.1 |
