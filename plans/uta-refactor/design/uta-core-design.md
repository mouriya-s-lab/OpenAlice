...
- **锁的归属（维护者）**：悲观锁与乐观锁的最大差异不是性能，是**锁住在谁那里**——悲观锁不侵入外部系统（锁在 UTA 内部持有，venue 不知道也不需要知道）；乐观锁的"锁"本身是外部系统的一部分，因为找到锁的路径（要比较的那个版本）不在我们的数据库里。由此：
  - **UTA 内部状态默认悲观**：lane 串行化（§4）、待决集合版本、单写者 SQLite 事务——锁在 UTA 内，找锁的路径在 SQLite，对 venue 零侵入。C11 的"待决集合版本期望"比较的是 UTA 自己拥有的状态，因此它是内部一致性检查，不是对外乐观锁。
  - **对外部状态（报价、venue 侧持仓、订单 listing）UTA 不拥有任何锁**。所谓"乐观校验"只是对证据新鲜度的有界赌注：它不阻止世界在 prepare 与 venue 执行之间变化（那是 F5/对账的领域），只保证"我们发出的写建立在没有过期、没有被撤回的读之上"。如果某 venue 提供真正的外部锁（cancel/replace 引用原单身份、条件单、expected-version 式改单、幂等键唯一性），那把锁是 **venue 的**，经能力证据（§7）声明后由 IO 壳使用；能力未声明则不存在，不得假装。
  - DB 隔离级别的词汇（可重复读、串行化）不适用于对外部分，不得借用。**[设计]**
