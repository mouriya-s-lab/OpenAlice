...
- **待裁决的推论**：H3 把凭据 owner 放在 Alice 侧封存配置，前提是 Alice 拉起 UTA 并注入。生命周期解绑后，凭据要么由 UTA 自己读取封存配置（H3 被证伪，owner 移到 UTA），要么经控制面在会话内注入并由 UTA 持久化。这是 C7 注入链的形状决定，不改变"程序与消费方只见账户身份"。
