...
- **对账驱动的自动化边界**：进入 `Undeterminable` 后，IO 壳按该 (venue, op) 能力证据声明的渠道**自动**依次取证（`by-key → listing + venue 身份 → fills/positions → 保留期内 replay-by-key`），每次取证 append 一条 `Evidence`。渠道返回 `found` 或 `absent` 即**自动收敛**；渠道穷尽仍 `inconclusive`，append 后**停下等待带 principal 的人工 `Evidence`**，IO 壳**永不 heuristic**。取证（读）可以重试，`submit`（写）永不重试。**[证据：fp-06 修正 7；域 C1/C2/C12]**
