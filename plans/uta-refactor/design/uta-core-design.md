...
**模型：与 Haskell 的 `IO` 几乎一样——先定义，再运行（维护者）**。`IO a` 是一个**描述**动作的值，构造它不做任何事；只有 runtime 在 `main` 处运行它时效果才发生。UTA 完全同构：
...
区别只有一处，且正是 F5：Haskell 的 runtime 总能知道一个 `IO` 动作做完没有；UTA 的 runtime 面对 venue 时可能不知道——所以 UTA 的"运行"多出一个 `Undeterminable` 结果与一段对账，其他一切与 `IO` 的纪律相同。**"先定义再运行"是 IO 壳的全部设计原则**：任何"运行"的东西先是一条记录；任何记录都不是运行。
...
- **单据锁在它之外**（§8b 属于意图形成期，与 IO 无关）；因此"UTA 唯一的锁"与"IO 壳内没有锁"两句同时成立。
...
- **与写边界的唯一接口**：`Close(Reserved(pos))`——决策链（§4）对 `Submitted(head)` 放行后 append `Reserved`，同一事务里 `Close`。这是单据锁与 IO 壳之间**唯一**的一条边，方向是单向的（单据 → 记录），IO 壳不知道单据的存在。
...
- **产品层代价**：两个 AI 不能同时处理同一张单据——合理，但不好用；核心有意接受，不用分叉/合并修补。协作在核心之外：`Handoff`；第二个 AI 另起单据由决定者二选一；或把建议发给负责人。
- **决定绑定 head**：Decision 引用 `Submitted(head)` 的 hash；进入 `Reserved` 要求被决定的版本 = head。`Return` 后 `Edit` 使 head 前进，旧 Decision 自然失效。
