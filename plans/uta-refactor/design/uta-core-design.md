...
- **两套存储各管各的**：段池不持久化，进程重启后由 `Pooled` 节点从其输入流重新洗入（或从段池自己的快照恢复，S15）；SQLite（§6.1）只持久化 `Journal`。`Pooled` 的输入流本身是否落 `Journal`（全量、抽样、还是只有 gap 标记）是那条流的 `Journal` 策略，与段池无关 → **[spike S15]**。非行情流不经段池。
...
| `Journal` / 段池 | 记录的载体，持久于 SQLite，由 §6 保留协议管理 | 为高性能计算设计的特殊物化，由 `Pooled` 洗入，自有回收，不持久 | 两套存储，互不派生；只共用 `LogPosition` 标定 | §2 §5.3 §6.1 |
...
| S15 | 段池的重启恢复（从输入流重洗 vs 段池自有快照）；`Pooled` 输入流自身的 `Journal` 策略（全量 / 抽样 / 仅 gap 标记）（§5.3） | D12 |
