...
- **互斥只在 UTA 自己消费的地方；对 venue 写入通道的顺序是协议语义（维护者 + 修正）**。提交是消费动作，只有有权消费者才谈得上锁。F1：venue 域事实（账户、持仓、订单、成交、价格）由 venue 消费与裁决，UTA 无权决定是否消费，因此对它们**不存在 UTA 的锁**。UTA 唯一自己消费的东西是**自己的日志**（审批、意图、尝试、队列顺序、程序装载、订阅表都是它的记录），互斥只在这一处：
