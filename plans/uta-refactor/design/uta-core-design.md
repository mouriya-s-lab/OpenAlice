...
- **意图在 `Reserved` 之前是值，不是状态**：`Intent` 有身份（draft id）与版本（内容 + 父版本的 hash）。"编辑"= 追加一个以**当前头**为父的新版本记录，带编辑者 principal 与依据 `Pos`；从不原地修改。
- **产品规则：一份草稿只有一种交易意图，不是两种（维护者）**。这与代码协作不同——代码可以分叉再合并，订单草稿不能"同时想买又想卖两个价"。因此**不允许分叉**：一个 draft 在任何时刻只有一个头；每次编辑都是对当前头的 CAS（expected-old-value = 编辑者读到的头 hash），头已变则该编辑**立即被拒**并返回新头，编辑者必须重读后再改。被拒的编辑不进入日志成为分支；只记一条"编辑冲突"事件供审计。两个 AI 同时编辑的结果是**一个赢、一个被告知重读**，不是两个版本并存。
- **决定绑定版本**：审批/规则放行的 Decision 引用具体的头 hash（C11 的"对待决集合版本的期望"）。进入 prepare 的规则要求：被 prepare 的版本 = 被决定的版本 = 当前头；不满足 → 冲突记录，不执行，需要重新决定。这是依据有效性（§8.2 第 3 条）在意图自身上的同一条规则。
- **一个 draft 至多一个版本到达 `Reserved`**：到达后该 draft 关闭；此后的改动只能是新意图（改单/撤单），各走各的写边界。
- **推论**：编辑冲突从来不需要锁，需要的是头可见——CAS 让后到者立刻知道头变了；谁的版本进入 prepare 由决定与规则说了算。
- **与 git 的关系（收窄）**：借用的只有**内容寻址的线性历史 + 对 ref 的 CAS**（谁先 push 谁是权威、非 fast-forward 拒绝）；**不借用分支与合并**——一个 draft 就是一个只允许 fast-forward 的 ref，编辑即 push。ref 是权威所在但不是锁：CAS 是一次原子比较，不持有、不阻塞。**[设计：维护者定性]**
