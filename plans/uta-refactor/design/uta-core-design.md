...
- **UTA 没有任何锁（维护者）**。提交是消费动作，只有有权消费者才谈得上锁。F1：venue 域事实（账户、持仓、订单、成交、价格）由 venue 消费与裁决，UTA 无权决定是否消费，因此对它们**不存在 UTA 的锁**。UTA 自己的记录（审批、意图、尝试、队列顺序、程序装载、订阅表）是 UTA 说出的话，UTA 对它们有**权威**，但权威不是锁：锁保护临界区不被交错，而 append-only 日志上顺序就是位置本身——单写者（H10）、每次 append 原子、没有第二个写者争同一位置，也没有横跨多个位置的临界区。"读状态 → 决定 → append"由决策引擎按队列依次驱动，没有并发决定者；过期读由 C11 的依据版本**可见**，不需要锁让它不可能。
...
### 8.3 暂存的意图是脚本，不是事务（维护者）

场景：两个 AI 同时编辑同一个尚未决定的暂存订单。这**不是任何数据库事务形态下的锁问题**——事务根本还没开始（prepare 在 `Reserved`），此时的意图只是一份还没执行的脚本，两个 AI 在改的是"将要提交的 SQL 文本"，不是在争一次提交。因此：

- **意图在 `Reserved` 之前是值，不是状态**：`Intent` 有身份（lineage id）与版本（内容 + 父版本的 hash）。"编辑"= 追加一个以旧版本为父的新版本记录，带编辑者 principal 与依据 `Pos`；从不原地修改。
- **并发编辑 = 分叉，不 = 冲突**：两个 AI 的编辑各成一个版本，父相同，都被记录；编辑时没有任何东西被消费，所以没有什么需要仲裁。
- **决定绑定版本**：审批/规则放行的 Decision 引用的是具体版本 hash（C11 的"对待决集合版本的期望"）。进入 prepare 的规则要求：被 prepare 的版本 = 被决定的版本，且是决定者当时所见 lineage 的最新版本；不满足 → 冲突记录，不执行，需要重新决定。这是依据有效性（§8.2 第 3 条）在意图自身上的同一条规则。
- **一条 lineage 至多一个版本到达 `Reserved`**：到达即其余分支被记为 superseded（Event），此后对该 lineage 的编辑只能是新意图（改单/撤单），走各自的写边界。
- **推论**：编辑冲突从来不需要锁，需要的是版本可见——两个 AI 各自看到对方的版本存在于日志里，谁的进入 prepare 由决定与规则说了算，不由谁先按下按钮说了算。**[设计：维护者定性]**
- **这就是 git 的模型（维护者）**：版本 = commit（内容寻址、带父指针），lineage = 分支，暂存编辑 = 在自己分支上提交，进入 prepare = push 到权威 ref。**谁先 push 谁是权威**：ref 只接受 fast-forward，C11 的"对待决集合版本的期望"就是 push 时的 expected-old-value（compare-and-swap），非 fast-forward 被拒绝，落后者必须基于新 ref 重新决定（rebase 后再 push，即重新审批）。ref 是待决集合的权威所在，但它不是锁：push 是一次原子 CAS，不持有任何东西、不阻塞任何人；两个 AI 各自提交多少版本都可以，只有 push 那一下分胜负。**[设计：维护者定性]**
