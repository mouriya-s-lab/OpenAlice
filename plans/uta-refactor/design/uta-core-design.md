...
### 5.3 `Pooled`：核心暴露给可选计算子系统的唯一接口（维护者约束）

**行情派生高性能计算子系统是可选项，独立于核心，不属于核心。**[设计：维护者定性]** 它的全部设计——段池存储模型、洗入、布局推导/导出/身份、契约表、段生命周期、原生 op 装载与握手、外部编译、选库（`iceoryx2`）、信任与故障域、50 ms 预算、向量化、实测闸门——在 `hpc-derivation-subsystem.md`；本节只定核心与它之间的接口，核心在没有该子系统时完整可运行。
...
- **前置条件**（装载期校验，不满足即拒绝）：`input` 的记录类型定长、位置线性、无指针、可容忍 ring 回收。quote/bar/tick 及其指标满足；余额/持仓/订单状态/新闻不满足，所以后者不能应用 `Pooled`——是条件不满足，不是架构隔离。
- **对核心的零影响**：`Journal`、`LogPosition`、§0.2 的五个 fold、§7.0 注册表的含义不变；`required_inputs` 遇到 `Pooled` 交给子系统解析，其余解析到普通流；效应侧、单据、投影、§6 保留协议不受影响。子系统未安装时，含 `Pooled` 的程序在装载期被拒绝，其余程序不受影响。
- **原生计算不是新节点种类**：它是 §7.0 注册表里由子系统提供的黑盒 op，要求输入是 `Pooled` 的；输出是一条派生观察流，下游节点像读任何派生流一样读它。"程序不是黑盒函数"对决策（解释②）继续成立。
- **消费者不变**：如"唤醒对应订单的 AI"是程序里的 `On(pattern) → Emit(EffectRequest)`；若产生外部写，走单据 → STS → IO 壳，不增设旁路。
- **三条与核心相关的裁决**（细节在子系统文档）：段池与 `Journal` 是两套存储，段池不持久，持久化行情归 `Journal`（D12、E6）；信任边界在安装期，不提供 sandbox，值树程序仍不可信（D11）；§5.2 的 Wasm 门槛只约束 Wasm 选项，与子系统无关。

子系统自身的 spike：S12（实测闸门）、S13（布局 fold 确定性、工具链缺口）、S14（触发通道）。
...
| `Pooled` 组合子 / 原生 op | `Pooled`：值树里的读侧组合子，核心暴露给可选子系统的唯一接口 | 原生 op：子系统提供的注册表黑盒，要求输入是 `Pooled` 的 | 前者属核心代数，后者属可选子系统；核心只见 op 的登记声明与输出段 | §5.3；子系统文档 |
...
| S12 | **选库已裁决 `iceoryx2`（fp-08）**；剩实测闸门：macOS/Windows 跨独立进程只读消费、借用期回收、reader 崩溃回收、Slice 重分配快照、契约表归一、端到端 50 ms 分项、列式段对比、原语集实证（`hpc-derivation-subsystem.md` §9） | fp-08 §7.1；fp-09 §7；fp-10 §4；维护者裁决 |
...
- **`research/fp-12-l2-orderbook-demo.md`**：L2 订单簿 demo（LOBSTER AMZN level-10）——档轴归约、多档字段布局 A/B、中间列物化的一手实测；子系统 §3.0/§3.1/§8.1 范围限定的证据。
...
| D12 | 行情派生计算子系统 | **可选、独立、不属于核心**；与核心唯一接口是 `DerivationNode::Pooled`。独立进程 + 只读共享内存映射（同物理页非同地址空间）；段池与 `Journal` 是两套存储、段池不持久；首选 `iceoryx2`；前置条件限定为行情类流（维护者） | §5.3；`hpc-derivation-subsystem.md` |
