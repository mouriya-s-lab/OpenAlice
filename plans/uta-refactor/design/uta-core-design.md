...
- **lane（UTA 自己的发出顺序范围 = unknown 阻塞半径；不是锁，见 §8.2）**：键 = **(账户, 子账户)**，无子账户的 venue 退化为账户。理由：这恰是 H4 要求的写全序范围；细于它（如按 instrument）防住了重复投放却防不住"资金状态未知时继续加仓"（H1 的真正含义：unknown 占用的 buying power 不按 instrument 隔离，venue 限流也按账户）；粗于它（整账户）超出 H4 且让一笔卡住的订单冻结无关子账户。子账户须先枚举（C10）。**[设计]**
...
- **UTA 不锁任何东西（维护者）**。UTA **没有权威副本**——账户、持仓、订单、成交、价格的权威全在 venue（F1），UTA 的记录都是解释。锁一份解释保护不了任何现实，所以悲观/乐观的二分对 UTA 都不适用：既不锁自己（无权威可锁），也不锁外部（不拥有）。之前把 lane 串行化、待决集合版本、单写者事务称为"UTA 内部悲观锁"是错的，收回。UTA 内部只有两件事，都不是锁：
  - **自己动作的顺序**：lane 是每 (账户, 子账户) 一条**发出序列**（H4：venue 侧顺序只能由我们的发出顺序决定），它排的是 UTA 自己的 Attempt，不排世界；unknown 阻塞是"在证据回来前不再发出"的自我约束，同样不是锁。
