...
- **UTA 不锁任何东西（维护者）**。UTA **没有权威副本**——账户、持仓、订单、成交、价格的权威全在 venue（F1），UTA 的记录都是解释。锁一份解释保护不了任何现实，所以悲观/乐观的二分对 UTA 都不适用：既不锁自己（无权威可锁），也不锁外部（不拥有）。之前把 lane 串行化、待决集合版本、单写者事务称为"UTA 内部悲观锁"是错的，收回。UTA 内部只有两件事，都不是锁：
  ...
  - **对外部状态无锁**："依据有效性"只是对证据新鲜度的有界赌注，不阻止世界在 prepare 与 venue 执行之间变化（那是 F5/对账的领域）。venue 若提供真正的锁（cancel/replace 引原单身份、条件单、expected-version 改单、幂等键唯一性），那把锁是 **venue 的**，经能力证据（§7）声明后由 IO 壳使用；未声明则不存在，不得假装。悲观锁不侵入外部系统、乐观锁的锁本身是外部系统一部分（找锁的路径不在我们的库里）——这个区分用来判断 venue 提供的是哪种，不用来给 UTA 自己造锁。
