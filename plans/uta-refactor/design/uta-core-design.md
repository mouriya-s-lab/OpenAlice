...
- **lane（对 venue 写入通道上的有序队列 = unknown 阻塞半径；队列有序才重要，不是锁，见 §8.2）**：键 = **(账户, 子账户)**，无子账户的 venue 退化为账户。理由：这恰是 H4 要求的写全序范围；细于它（如按 instrument）防住了重复投放却防不住"资金状态未知时继续加仓"（H1 的真正含义：unknown 占用的 buying power 不按 instrument 隔离，venue 限流也按账户）；粗于它（整账户）超出 H4 且让一笔卡住的订单冻结无关子账户。子账户须先枚举（C10）。**[设计]**
- **阻塞什么不进键、进规则**：默认——lane 内存在未收敛的 `Undeterminable` 时阻塞该 lane 全部新 Attempt（C1 保守读法）。例外（如撤单/减仓类放行）是顺序固定链里一条独立、可审计的规则；带 principal 的显式绕过记为 Decision，不静默。键稳定，策略可改。**[设计]** **推论：队列不是纯 FIFO。** 队首阻塞会把降低风险的撤单也压在后面，而"放行"= 允许后来者越过队首。因此 lane 队列是**两类**：阻塞类（默认）与可越过类（由放行规则标记）；可越过类不改变阻塞类之间的相对顺序（H4 的全序只对同类保证），且越过本身记录为 Event 以便审计。**[设计]**
...
**命名限定**：2PC 有两半——(a) 中间态语义；(b) 参与者承诺（prepare 成功即承诺能 commit，协调者裁决、参与者服从，含 heuristic commit/rollback）。venue 只给 (a) 不给 (b)：venue 从不 prepare，UTA 不能裁决 venue 做了什么、只能发现。UTA 实现的是**协调者半边的 write-ahead + in-doubt 决议，没有参与者半边**；决议出口只有 found / absent / inconclusive，没有 heuristic。文中"两阶段协议"一律按此限定理解，不得引入 (b) 的假设。**[设计]**
...
- **UTA 只对自己拥有的资源做互斥，对 venue 域事实不做任何锁（维护者 + 修正）**。F1 说的是 **venue 域事实**（账户、持仓、订单、成交、价格）的权威在 venue，UTA 的记录都是解释，锁一份解释保护不了现实。但 UTA 对**自己的承诺**是唯一权威：审批是否批准、哪个 Attempt 先发出、lane 队列顺序、程序装载、订阅表——这些 venue 不知道也不拥有。因此悲观/乐观的二分对 venue 域事实不适用（既无权威可锁，也不拥有外部），而 UTA 自有资源上的互斥是正当的、且只有两处：
  - **自己动作的顺序 = 写入通道上的队列**：串行化只属于日志本身（append 的位置即顺序）。对 venue 的写入通道（每 (账户, 子账户) 一条，H4：venue 侧顺序只能由我们的发出顺序决定）是一条**有序队列**："锁"在这里只是抢占写入通道、让自己先写的说法，抢占的结果自然形成阻塞队列——队首在等证据（`Undeterminable` 未收敛）时后面的 Attempt 排队等待，这就是 unknown 阻塞的全部机制。重要的是**队列有序**，不是锁；lane 因此定义为写入通道的队列，不是锁原语。
  ...
  - **对外部状态无锁**："依据有效性"只是对证据新鲜度的有界赌注，不阻止世界在 prepare 与 venue 执行之间变化（那是 F5/对账的领域）。venue 若提供真正的锁（cancel/replace 引原单身份、条件单、expected-version 改单、幂等键唯一性、保证金预占），那把锁是 **venue 的**，经能力证据（§7）声明后由 IO 壳使用；未声明则不存在，不得假装。判断"锁在谁那里"的轴是**被比较/被持有状态的权威归属**，不是悲观/乐观（venue 侧保证金预占是悲观且外部，DB 版本列是乐观且本地）。
...
- **意图与决定的权威不同**：AI/程序/审批人都由读触发（因果同形），但**授权不同形**——程序输出的 Intent 没有权威，必须经规则或审批才成为 Decision；审批人的 Decision 是带 principal 的授权事实（S8："谁、何时、依据什么"）。§0 的"决定"一步因此内含两类记录：派生的意图与授权的决定；不得把审批压平成一种派生观察。**[设计]**
