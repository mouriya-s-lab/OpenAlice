...
- **两层输出**：集成输出分为 **raw evidence**（原始字节永远保留，C13）与 **validated payload**（入口解析为内部小词表；parse-don't-validate，隐藏构造器）。venue 状态映射按 venue 枚举输入，输出**必须保留 `Unmapped(raw)`**，不能靠"无 catch-all"伪造穷尽映射。**[证据：fp-04 命题 15/16；域 C13/F6]**
...
- **集成的职责由 IDL 固定**：能力握手（§7 `Capability`）、按 range 位置推进观察记录、raw evidence + validated payload 双层输出、响应投放与对账查询（仅限核心 IO 壳调用）、上报 SourceGap。集成**不持有**规则状态、不做决策、不接触 SQLite。
